...
class App extends  Component {
 ...
 componentDidMount () {
  fetch(`https://dockerized-magicbrainapi.onrender.com/signin`, {
  	method: 'post',
  	headers: {
   ...
   .then(data => {if(data && data.id){
     fetch(`https://dockerized-magicbrainapi.onrender.com/profile/${data.id}`, {
      method: 'get',
  	  headers: {
      'Content-Type': 'application/json',
      'Authorization': window.sessionStorage.getItem('token')
  ...
  onButonSubmit = ()=>{
     ...
     fetch(
      `https://dockerized-magicbrainapi.onrender.com/imageUrl/${this.state.user.id}`,
       { method: 'put',
         headers: {
          'Content-Type': 'application/json',
          'Authorization': window.sessionStorage.getItem('token')
         ...
         .then(response => {
            if(response){
              ...
              fetch(`https://dockerized-magicbrainapi.onrender.com/image/${this.state.user.id}`, {
                method: 'put',
                headers: {
                  'Content-Type': 'application/json',
                  'Authorization': window.sessionStorage.getItem('token')
